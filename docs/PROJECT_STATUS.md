...
- 实现：本地 Caption/VLM（默认 Florence-2-base）生成图片描述；远程文本 LLM 判定描述与用户文本一致性；远程多模态判定（glm-4v-flash，OpenAI 兼容接口调用，模式由 `VISION_PIPELINE_MODE` 控制）；调试日志展示 caption。
- 不足：远程文本/多模态缺少超时、重试、频控与回退；多模态返回 JSON 解析/格式鲁棒性有限；无置信度或多次采样；UI 仅展示基本状态。
- 下一步：完善远程一致性验证（文本与多模态路径的稳健性与安全策略）。
