...
- 实现：本地 Caption/VLM（默认 Florence-2-base）生成图片描述；远程文本 LLM 判定描述与用户文本的一致性（CONSISTENT/INCONSISTENT）；可选远程多模态直接判定图+文；调试日志展示 caption。
- 不足：判定依赖外部模型，未做超时/频控/回退；无置信度或多次采样；多模态分支默认关闭且未校验出错路径；UI 仅展示基本状态。
- 下一步：完善远程一致性验证（多模态/文本判定的稳定性、回退与安全策略）。
